...
- Publish a GitHub release with `npm run release:github`. The worktree must be clean and `gh` must be authenticated. The script builds the selected artifact, creates and pushes tag `vX.Y.Z`, creates the GitHub release with the artifact, increments the patch version in `package.json`, `app.json`, and `package-lock.json`, commits the version bump, and pushes it to the current branch. Pass `aab` to publish an Android App Bundle instead of the default APK.
